...
maap f = foldRight (\ele lst -> (f ele) :. lst) Nil
...
fiilter p = foldRight (\ele lst -> if p ele then ele :. lst else lst) Nil
...
seqOptional = foldRight (<:.>) Empty
  where
    (<:.>) :: Optional a -> Optional (List a) -> Optional (List a)
    (Full a) <:.> Empty      = Full (a :. Nil)
    (Full a) <:.> (Full lst) = Full (a :. lst)
    Empty    <:.> lst        = lst
...
-- prop> (rev . rev) x == x
rev :: List a -> List a
rev = foldLeft (\acc ele -> ele :. acc) Nil
